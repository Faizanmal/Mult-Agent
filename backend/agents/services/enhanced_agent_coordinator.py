...
class EnhancedAgentCoordinator:
    ...
    def _determine_relevant_agents_enhanced(self, message: Message) -> List[Agent]:
        ...
        needs_vision = 'image' in content or 'picture' in content or 'photo' in content or 'visual' in content or message.message_type == 'image'
        needs_reasoning = 'think' in content or 'reason' in content or 'analyze' in content or 'logic' in content or 'problem' in content
        needs_action = 'do' in content or 'execute' in content or 'run' in content or 'perform' in content or 'task' in content
        needs_memory = 'remember' in content or 'recall' in content or 'store' in content or 'save' in content or 'history' in content
        
        # Select best agents using smart selection
        if needs_vision:
            vision_agent = self.agent_selector.select_best_agent('vision', content, session_agents.filter(type='vision'))
            if vision_agent and vision_agent not in relevant_agents:
                relevant_agents.append(vision_agent)
        
        if needs_reasoning:
            reasoning_agent = self.agent_selector.select_best_agent('reasoning', content, session_agents.filter(type='reasoning'))
            if reasoning_agent and reasoning_agent not in relevant_agents:
                relevant_agents.append(reasoning_agent)
        
        if needs_action:
            action_agent = self.agent_selector.select_best_agent('action', content, session_agents.filter(type='action'))
            if action_agent and action_agent not in relevant_agents:
                relevant_agents.append(action_agent)
        
        if needs_memory:
            memory_agent = self.agent_selector.select_best_agent('memory', content, session_agents.filter(type='memory'))
